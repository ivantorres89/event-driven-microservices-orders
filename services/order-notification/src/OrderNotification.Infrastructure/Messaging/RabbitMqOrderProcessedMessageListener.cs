...
public sealed class RabbitMqOrderProcessedMessageListener : BackgroundService, IOrderProcessedMessageListener
{
    private static readonly TextMapPropagator Propagator = Propagators.DefaultTextMapPropagator;

    private readonly RabbitMqOptions _options;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<RabbitMqOrderProcessedMessageListener> _logger;
    private readonly IConnectionFactory _connectionFactory;
    private readonly JsonSerializerOptions _json;

    public RabbitMqOrderProcessedMessageListener(
        IOptions<RabbitMqOptions> options,
        IServiceScopeFactory scopeFactory,
        ILogger<RabbitMqOrderProcessedMessageListener> logger)
    {
    ...
    internal async Task HandleMessageAsync(IChannel channel, BasicDeliverEventArgs ea, CancellationToken ct)
    {
        ...
        OrderProcessedEvent? message = null;

        try
        {
            var json = Encoding.UTF8.GetString(ea.Body.ToArray());
            message = JsonSerializer.Deserialize<OrderProcessedEvent>(json, _json);
            ...
            await handler.HandleAsync(new NotifyOrderProcessedCommand(message), ct);
