...
This repository uses a **GitHub Actions** workflow that focuses on **Continuous Integration (CI)**: it validates that the codebase builds, tests pass, and quality signals (coverage + reports) are published as artifacts.  
It intentionally does **not** perform Continuous Delivery/Deployment (CD) because this is a **portfolio/demo** project—running and maintaining always-on cloud infrastructure (registries, clusters, environments, approvals, secrets rotation, etc.) would add recurring cost and operational overhead that are **out of scope**.

### What the pipeline does

At a high level, the workflow provides **reproducible validation** for both backend and frontend:

- **Full Git checkout** (`fetch-depth: 0`) so versioning tools can compute accurate versions.
- **.NET build (Release)** for all three backend services.
- **Backend unit tests** discovered automatically (`*.UnitTests.csproj`) with **XPlat Code Coverage** and `trx` test results.
- **Backend integration tests** against real dependencies started with Docker Compose:
  - RabbitMQ, Redis and SQL Server are started and waited-on until healthy.
  - Database migrations are executed **once** against a clean integration-test database.
  - Integration tests run for `order-accept`, `order-process` and `order-notification`, also with coverage + `trx`.
  - Infrastructure is always torn down (`docker compose down -v`) even if tests fail.
- **Frontend (Angular) build** and **unit tests** (runs only if `*.spec.ts` files are present).
- **Coverage reporting** via ReportGenerator:
  - A Markdown summary is published into the GitHub Actions **job summary**.
  - Coverage and test results are uploaded as **artifacts** for traceability.
  - A coverage **badge** is updated on the `badges` branch (requires `contents: write` permissions).

The result is a pipeline that is fast enough for frequent iteration, but also strong enough to demonstrate **real-world engineering habits**: automated builds, layered tests, integration validation with real infrastructure, and visible quality metrics.

### Why there is no CD here (and how it would look)

For this portfolio repository, the goal is to demonstrate architecture and engineering practices without binding the project to a paid cloud footprint.

That said, the workflow is already structured in a way that makes CD a natural next step:

- Build Docker images per service and tag them using the **SemVer** generated by GitVersion.
- Authenticate to **Azure Container Registry (ACR)** (ideally with GitHub → Azure **OIDC** and least-privilege roles) and push images.
- Grant the Kubernetes runtime identity `AcrPull` so **AKS can pull images from ACR**.
- Deploy with Helm/Kustomize (or GitOps with Argo/Flux) and add environment gates/approvals if desired.

Those steps are deliberately omitted here to keep the repo lightweight and cost-free, but the existing CI foundation is designed so that adding delivery is mostly a matter of wiring credentials + deployment strategy.

### GitVersion

This workflow uses **GitVersion** to compute a deterministic **Semantic Version (SemVer)** for every CI run.

Key points of the configuration (`GitVersion.yml`):

- **Mode:** `ContinuousDelivery` — versions evolve automatically based on Git history and branch context.
- **Tag prefix:** only tags like `v1.2.3` / `V1.2.3` are treated as version tags (environment/promotion tags are ignored).
- **Branch rules:**
  - `main` increments **Patch** by default.
  - Pull request branches are labeled with `PR` and inherit increment rules.

The computed `semVer` is then reused consistently across the workflow (job summary, artifacts, and coverage badge naming). In a future CD extension, the same SemVer would also be the natural tag for container images and release artifacts.
...
- After starting the portal, open `http://localhost:3000` and browse:
  - **Catalog → Components** (services + frontend)
  - **Catalog → Systems** (`contoso-orders`)
  - **Docs** tab on each component (runbooks)

- Get it up:
  .\scripts\backstage-start.ps1
- Stop it:
